...
void resetUniformGrid( const std::vector< RigidBody2D* >& in_RigidBodies, const double in_ugrd_dx, UniformGrid2D** io_Ugrd )
{
...
void rasterizeObjects( const std::vector< RigidBody2D* >& in_RigidBodies, UniformGrid2D* io_Ugrd )
{
...
bool proxyCollisionDetection( const RigidBody2D* in_Body1, const RigidBody2D* in_Body2 )
{
...
void narrowPhaseCollisionDetection( RigidBody2D* io_TargetBody, const RigidBody2D* in_BodyToBeTested, const int in_frame_idx )
{
  for( int i=0; i<io_TargetBody->numCollisionSamples(); i++ )
  {
    ...
    if( sd < 0.0 )
    {
      auto p = sample.collision_cache.find( in_BodyToBeTested->index() );
      ...
      else
      {
        ...
        sample.collision_cache.insert( std::pair< int, CollisionInfo2D >( in_BodyToBeTested->index(), info ) );
      }
    }
  }
}

void clearOldCollisionInfo( std::vector< RigidBody2D* >& io_RigidBodies, const int in_frame_idx )
{
...
void broadPhaseCollisionDetection( std::vector< RigidBody2D* >& io_RigidBodies, const double in_ugrd_dx, UniformGrid2D** io_Ugrd, BroadPhaseActiveSet& io_BroadPhaseActiveSet )
{
...
void collisionDetection( std::vector< RigidBody2D* >& io_RigidBodies, const double in_ugrd_dx, UniformGrid2D** io_Ugrd, BroadPhaseActiveSet& io_BroadPhaseActiveSet, const int in_frame_idx )
{
  ...
  for( int j=0; j<io_RigidBodies.size(); j++ )
  {
    ...
    {
      int idx_i = *q;
      if( io_RigidBodies[idx_i]->isStatic() && io_RigidBodies[j]->isStatic() ) continue;
